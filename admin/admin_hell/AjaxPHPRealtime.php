<?php
include("../config.php");
$sqli="SELECT * from orders inner join order_list on (orders.order_id=order_list.order_id) where (orders.foodstatusid_hell='1' or orders.foodstatusid_hell='2')and order_list.food_statusid='1' and order_list.food_typeid='6' GROUP BY order_list.order_id";
      ...
      <table class="table table-hover table-bordered" id="example" width="100%">
        <thead style="background-color: #35475e">
          ...
          <tbody>
              ...
              $total=0;
              $i=0;
              $strSQL="SELECT * from order_list inner join orders on (order_list.order_id=orders.order_id) inner join food on (order_list.food_id=food.food_id) where order_list.food_statusid='1' and order_list.food_typeid='6' and orders.table_id='".$rowi["table_id"]."' ";
              ...
              <tr>
                ...
                </td> <!-- ให้อัพเดทตาราง orders foodstatusid_hell=2 และ อัพเดทตาราง order_list=2 -->
              </tr>
				  </tbody>
      </table>
    </form>
