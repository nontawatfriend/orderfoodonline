...
<div class="title" style="text-align: center;">ออร์เดอร์ทั้งหมด</div>

<?php
	$sqli="SELECT * from orders inner join order_list on (orders.order_id=order_list.order_id) GROUP BY order_list.order_id order by order_list.list_id desc";
	$resulti=$db->query($sqli);
	while($rowi=$resulti->fetch_array(MYSQLI_BOTH)){?>
			<h4 class="page-banner-sm"><b>โต๊ะที่ <?php echo $rowi["table_id"]?> วันที่ <?php echo $rowi["order_date"]?></b></h4>
				...
				<form action="?page=check_bill" method="POST">
						...
						$total=0;
						$i=0;
						$sql="SELECT * from order_list inner join orders on (order_list.order_id=orders.order_id) inner join food on (order_list.food_id=food.food_id) inner join food_status on (order_list.food_statusid=food_status.food_statusid) where order_list.order_id='".$rowi["order_id"]."' and order_list.food_typeid='3' and orders.table_id='".$rowi["table_id"]."' order by order_list.list_id asc";
						$result=$db->query($sql);
						...
						$sql="SELECT order_list.order_id,orders.table_id,orders.food_statusid,order_list.food_typeid,order_list.order_unit,food.food_name,food_status.food_statusname,order_list.food_statusid,food.food_id,order_list.food_id, SUM(order_list.order_unit) as order_unit from order_list inner join orders on (order_list.order_id=orders.order_id) inner join food on (order_list.food_id=food.food_id) inner join food_status on (order_list.food_statusid=food_status.food_statusid) where  order_list.food_typeid<>'3' and orders.table_id='".$rowi["table_id"]."' and orders.order_id='".$rowi["order_id"]."' Group by order_list.food_id,order_list.food_statusid order by order_list.food_typeid DESC";//คือรวมให้เป็นชื่อเดียวกันแล้ว
						$result=$db->query($sql);
						while($row=$result->fetch_array(MYSQLI_BOTH)){
						?>
						<tr>
